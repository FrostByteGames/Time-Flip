...
class SteamManager : MonoBehaviour {
	...
	public static string steamIDstring { get; private set; }
	...
	private void Awake() {
		...
		// Cache some information about the steam account that is currently logged in
		steamID = SteamUser.GetSteamID();
		steamIDstring = steamID.ToString();
		displayName = SteamFriends.GetPersonaName();

		/* // AVATAR FUNCTION - untested
		byte[] b = new byte[128];
		if (SteamUtils.GetImageRGBA (SteamFriends.GetSmallFriendAvatar (steamID), b, 8192)) {
			uint avatarWidth = new uint ();
			uint avatarHeight = new uint ();
			if (SteamUtils.GetImageSize(SteamFriends.GetSmallFriendAvatar (steamID), out avatarWidth, out avatarHeight)) {
				Texture2D avatar = new Texture2D (32, 32);
				avatar.LoadRawTextureData (b);
			}
		} */


		Debug.Log("SteamAPI successfully initialised. SteamID: " + steamID + " ('" + displayName + "')");
	...
	private void OnGameOverlayActivated (GameOverlayActivated_t pCallback) {
		if(pCallback.m_bActive != 0) {
			...
			// Pause game here
			gameController.SteamOverlayActivated ();
		}
		else {
			Debug.Log("Steam Overlay has been closed");
			// Resume game here? or maybe leave the player to manually resume...
		}
	}
	...
	public static Texture2D GetSmallAvatar (string steamID) {
		Debug.Log ("Fetching Steam avatar for steamID " + steamID);

		CSteamID csteamID = new CSteamID (Convert.ToUInt64 (steamID));
		int FriendAvatar = SteamFriends.GetSmallFriendAvatar(csteamID);
		uint ImageWidth;
		uint ImageHeight;
		...
		if (success && ImageWidth > 0 && ImageHeight > 0) {
			...
			Debug.LogError ("    > Couldn't get Steam avatar!");
